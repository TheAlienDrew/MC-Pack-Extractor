...
case "$version" in
	("1.0" | "1.1" | "1.2.1" | "1.2.2" | "1.2.3" | "1.2.4" | "1.2.5" | "1.3.1" | "1.3.2" | "1.4.2" | "1.4.4" | "1.4.5" | "1.4.6" | "1.4.7")
		...
		if ($use7z); then
			7z x "$mcverdir/$version/$version.jar" -o"/tmp/mctemp" pack.png pack.txt particles.png terrain.png font.txt achievement armor art environment font gui item lang misc mob terrain title -r >/dev/null 2>&1
		else
			unzip -q "$mcverdir/$version/$version.jar" pack.png pack.txt particles.png terrain.png font.txt achievement/*.* armor/*.* art/*.* environment/*.* font/*.* gui/*.* item/*.* lang/*.* misc/*.* mob/*.* terrain/*.* title/*.* -d "/tmp/mctemp"
		fi ;;
	("1.5.1" | "1.5.2")
		echo -e "Extracting $version texture pack..."
		if ($use7z); then
			7z x "$mcverdir/$version/$version.jar" -o"/tmp/mctemp" pack.png pack.txt particles.png font.txt achievement armor art environment font gui item lang misc mob textures title -r >/dev/null 2>&1
		else
			unzip -q "$mcverdir/$version/$version.jar" pack.png pack.txt particles.png font.txt achievement/*.* armor/*.* art/*.* environment/*.* font/*.* gui/*.* item/*.* lang/*.* misc/*.* mob/*.* textures/*.* title/*.* -d "/tmp/mctemp"
		fi ;;
	("1.6.1" | "1.6.2" | "1.6.4")
		echo -e "Extracting $version resource pack..."
		if ($use7z); then
			7z x "$mcverdir/$version/$version.jar" -o"/tmp/mctemp" pack.png pack.mcmeta font.txt assets/minecraft -r >/dev/null 2>&1
		else
			unzip -q "$mcverdir/$version/$version.jar" pack.png pack.mcmeta font.txt assets/minecraft/*.* -d "/tmp/mctemp"
		fi ;;
	("1.7.2" | "1.7.3" | "1.7.4" | "1.7.5" | "1.7.6" | "1.7.7" | "1.7.8" | "1.7.9" | "1.7.10" | "1.8" | "1.8.1" | "1.8.2" | "1.8.3" | "1.8.4" | "1.8.5" | "1.8.6" | "1.8.7" | "1.8.8" | "1.8.9" | "1.9" | "1.9.1" | "1.9.2" | "1.9.3" | "1.9.4" | "1.10" | "1.10.1" | "1.10.2" | "1.11" | "1.11.1" | "1.11.2" | "1.12" | "1.12.2" | "1.13" | "1.13.1" | "1.13.2" | "1.14" | "1.14.1") 
		echo -e "Extracting $version resource pack..."
		if ($use7z); then
			7z x "$mcverdir/$version/$version.jar" -o"/tmp/mctemp" pack.png pack.mcmeta assets/minecraft -r >/dev/null 2>&1
		else
			unzip -q "$mcverdir/$version/$version.jar" pack.png pack.mcmeta assets/minecraft/*.* -d "/tmp/mctemp"
		fi ;;
	(*)
...
if ($use7z); then
	7z a "$origindir/MC$version.zip" "/tmp/mctemp/*" >/dev/null 2>&1
else
	cd "/tmp/mctemp" && zip -qr "$origindir/MC$version.zip" * && cd "$origindir"
fi
